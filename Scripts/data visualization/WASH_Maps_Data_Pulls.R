...
data.frame.validated %>% 
  dplyr::select("country_name", "country_ID", "governorate_ID", 
                "governorate_name", "district_ID", "district_name", 
                "market_name", "distance_travelled","type_area") -> med.time

#Aggregate medians
med.time %>% aggregate_median("district_ID")->median.time.file

View(median.time.file)
write.csv(median.time.file, file = paste0("Outputs/WASH Maps/distance_water_time_map_",current_month,".csv"), row.names = FALSE)
